...
export const FloatingActionPanelContext = React.createContext<
...
export function FloatingActionPanelTrigger({
  ...
  const handleClick = () => {
    if (triggerRef.current) {
      ...
      console.log('Opening panel at:', rect, 'mode:', mode);
      openPanel(rect, mode);
      setTitle(title);
    }
  };
...
export function FloatingActionPanelContent({
  ...
  React.useEffect(() => {
    console.log('FloatingActionPanelContent render - isOpen:', isOpen, 'triggerRect:', triggerRect);
  }, [isOpen, triggerRect]);
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className={cn(
              "fixed z-50 min-w-[200px] overflow-hidden rounded-lg border border-purple-200 bg-white shadow-xl outline-none",
              className
            )}
            style={{
          ...
          >
            <div className="px-4 py-3 font-semibold text-purple-700 border-b border-purple-200">{title}</div>
            {children}
          </motion.div>
        </>
      )}
...
export function FloatingActionPanelButton({
  ...
  return (
    <motion.button
      className={cn(
        "flex w-full items-center gap-2 rounded-md px-3 py-2 text-sm text-gray-700 hover:bg-purple-50 transition-colors text-left",
        className
      )}
      onClick={handleClick}
      whileTap={{ scale: 0.98 }}
